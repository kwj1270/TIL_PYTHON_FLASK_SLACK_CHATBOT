...
os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = "./chatbot-wmsa-95ab0990c036.json"

message_counts = {} # 사람당 메시지 몇개 했는지 셈
welcome_messages = {} # 사람당 웰컴 메시지 보냈는지 기록 
BAD_WORDS = ['욕', 'fuck', 'ㅅㅂ.'] # 금지어 사용
...
class WelcomeMessage:
    START_TEXT = {
        ...
        'text': {
            ...
            'text': (
                '스마트미디어학과 워크스페이스 채널에 오신 것을 환영합니다! \n\n'
                '*웰컴 메시지를 완료하시려면 이모티콘을 보내주세요!*'
            )
        }
    }
    ...
    if user_id != None and BOT_ID != user_id: 
        ...
        if text.lower() == 'start': # start 라는 글이 들어왔다면  
            ...
            client.chat_postMessage( # 메시지 보내기 
                channel=channel_id, thread_ts=ts, text="나쁜 글자는 사용하지 말아요 ㅠㅡㅠ!")
...
def webhook():
    ...
    text = req.get('queryResult').get('action')
    send_message(text, f'@{user_id}')

def send_message (text, channel_id) : 
    project_id = os.getenv( 'DIALOGFLOW_PROJECT_ID_')
    fulfillment_text = detect_intent_texts (project_id, " unique ", text, 'ko') 
    response_text = {"message ": fulfillment_text}
    client.chat_postMessage(channel=channel_id, text = fulfillment_text)    
